function mostrarSigno(){
    ...
    if(signo == 'ARI'){
        ...
        if(signo == ""){
            fragmentoHTML = '<p> Debes de seleccionar un personaje </p>'

        }else{
        fragmentoHTML = `<p> tu Personaje es ${nombreCaballero} </p>
        <img src="/imagenes/${nombreImagen}" alt="">
        <audio src="/audio/${nombreAudio}" autoplay></audio>
        <video src="/video/${nombreVideo}" autoplay replay controls></video>
        <button class = "cerrar" onclick="cerrar()">Cerrar</button>`
        }
...
function cerrar(){
    document.getElementById('caballero-oro').innerHTML = 
    document.getElementById('caballero-oro').style.display = 'none'
}
